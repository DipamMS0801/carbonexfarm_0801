...
function FarmerGenerate({ user, setUser, showNotif }) {
  ...
  const listCredits = () => {
    ...
    if(u){u.credits=(u.credits||0)+result.credits;u.listings=(u.listings||0)+1;u.points=(u.points||0)+50;}
...
function CompanyDashboard({ user, setUser, showNotif }) {
  const [tab,setTab]=useState("overview");
  const tabs=[{id:"overview",label:"Overview",icon:"home"},{id:"marketplace",label:"Marketplace",icon:"shop"},{id:"portfolio",label:"Portfolio",icon:"chart"},{id:"blockchain",label:"Blockchain",icon:"chain"}];
  return (
    <div style={{ padding:24,maxWidth:1200,margin:"0 auto" }}>
      <div style={{ display:"flex",gap:4,marginBottom:28,background:"var(--surface)",borderRadius:12,padding:6,border:"1px solid var(--border)",width:"fit-content" }}>
        {tabs.map(t=><button key={t.id} className={`tab-btn${tab===t.id?" active":""}`} onClick={()=>setTab(t.id)}><Icon name={t.icon} size={14}/>{t.label}</button>)}
...
function CompanyMarketplace({ user, setUser, showNotif }) {
  ...
  const buy = async (listing) => {
    ...
    const buyer=DB.users.find(x=>x.id===user.id); if(buyer){buyer.points=(buyer.points||0)+50;buyer.totalPurchased=(buyer.totalPurchased||0)+listing.credits;buyer.totalSpend=(buyer.totalSpend||0)+listing.totalValue;}
    const seller=DB.users.find(x=>x.id===listing.farmerId); if(seller){seller.points=(seller.points||0)+100;seller.revenue=(seller.revenue||0)+listing.totalValue;seller.totalSold=(seller.totalSold||0)+listing.credits;}
    setUser({...user,points:(user.points||0)+50,totalPurchased:(user.totalPurchased||0)+listing.credits});
    setBuying(null);
